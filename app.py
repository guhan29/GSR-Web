...
def getGsrResult():
    ...
    dataDict = datasetRef.get()
    # print(dataDict.values())
    dValues = dataDict.values()
    sample_array = np.fromiter(map(lambda d: float(d["gsrValue"]), dValues), dtype=float)
    dValues = list(dValues)
    print("Time values")
    # print(dValues[0], dValues[-1])
    t1 = datetime.datetime.strptime(dValues[0]['time'], "%H:%M:%S")
    t2 = datetime.datetime.strptime(dValues[-1]['time'], "%H:%M:%S")
    delta = t2 - t1
    print("Total Seconds", delta.total_seconds())
    # TODO: CALCULATE VITAMIN-D
    ans = dtw_algorithm(gsr_value_without_connection, sample_array)
    # print(ans)
    # print("---------------------------------------------")
    # print(ans[0][1])
    print("result")
    return jsonify({"status":ans[0][1], "delta": delta.total_seconds()})
    # print("---------------------------------------------")
    # return str(dtw_algorithm(gsr_value_without_connection, sample_array))
